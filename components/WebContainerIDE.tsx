...
import LocationSync from './LocationSync'; // Import the new component

export default function App() {
  return (
    <div>
      {/* This component will send path changes to the parent */}
      <LocationSync />
...
@tailwind utilities;`,
      ...
      // --- FIX: New component to send messages from the iframe ---
      "LocationSync.jsx": {
        file: {
          contents: `import { useEffect } from 'react';
import { useLocation } from 'react-router-dom';

// This is a "side-effect" component; it renders nothing.
// Its only job is to watch for location changes and notify the parent window.
...
export default function WebContainerIDE() {
  ...
  const [filesContent, setFilesContent] = useState({
    ...
    "src/pages/HomePage.jsx": files.src.directory.pages.directory["HomePage.jsx"].file.contents,
    "src/pages/PricingPage.jsx": files.src.directory.pages.directory["PricingPage.jsx"].file.contents,
    "src/LocationSync.jsx": files.src.directory["LocationSync.jsx"].file.contents,
  });
  ...
  useEffect(() => {
    // --- FIX: Add event listener to receive messages from the iframe ---
    const handleMessage = (event: MessageEvent) => {
      // Security: Only accept messages from the WebContainer's origin
  ...
  }, [baseUrl]); // Rerun this effect if the baseUrl changes
